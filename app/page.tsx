...
function AppContent() {
  const { connect, disconnect, account, connected, wallets: availableWallets } = useWallet();

  const handleConnect = async () => {
    try {
      const petra = availableWallets.find(w => w.name === 'Petra');
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white font-sans">
      <nav className="flex justify-between items-center px-10 py-5 border-b border-[#1a1a1a]">
        ...
        {connected && addr ? (
          ...
          <button onClick={handleConnect} className="bg-orange-500 hover:bg-orange-600 px-6 py-2 rounded-lg font-semibold transition">Connect Wallet</button>
        )}
      </nav>
      ...
      <div className="text-center pt-24 pb-16 px-10">
        ...
        {!connected ? (
          <button onClick={handleConnect} className="bg-orange-500 hover:bg-orange-600 px-10 py-4 rounded-xl text-lg font-bold transition">Connect Wallet to Start</button>
        ) : (
          <div className="inline-block bg-green-900/30 border border-green-500/30 px-8 py-3 rounded-xl">
            <span className="text-green-400 font-bold">✅ Connected: {addr?.slice(0,6)}...{addr?.slice(-4)}</span>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {models.map((m) => (
            <div key={m.name} className="bg-[#111] border border-[#1e1e1e] rounded-2xl p-6 hover:border-orange-500/30 transition">
              ...
              <button onClick={!connected ? handleConnect : undefined} className="w-full bg-[#1a1a1a] hover:bg-[#252525] border border-[#333] py-2 rounded-xl text-sm transition">
